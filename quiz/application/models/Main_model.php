...
class Main_model  extends CI_Model  {
    ...
    {		
    	$this->db->limit(1);
		$query = $this->db->get_where('tbl_admin', array('username' => $this->input->post('user_name'),'password' => $this->input->post('user_pass')));
    ...
    {		
    	$this->db->limit(1);
		$query = $this->db->get_where('tbl_user', array('username' => $this->input->post('user_name'),'password' => $this->input->post('user_pass')));	
		if ($query->num_rows() > 0)
		{
			$row = $query->row();
			return $row;
	...
	function submit_answer()
    {
    	...
    	// print_r($question);
    	// exit();
    	//Confirm that answer has not been submitted already for this question.
