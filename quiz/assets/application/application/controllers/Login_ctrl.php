...
class Login_ctrl extends CI_Controller {
	...
	public function index()
	{
		...
		{
			//check if the referrer is not empty then redirect to the referrer page
			// if(!empty($this->session->go_to)){
				$page = $_SESSION['go_to'];
	 ...
	 public function pass_check($password)
    {
		...
		else
		{
			...
			//set cookie
			//set_cookie('login_username',$this->input->post('user_name'),'100000000');
